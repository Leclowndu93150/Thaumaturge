...
public final class BoreDigEffect implements IFXInstance {
    ...
    public BoreDigEffect(
            ClientLevel level, BlockPos target, int boreEntityId, BlockPos borePos, BlockState state, int delay) {
        this.level = level;
        this.target = target;
    ...
    private @Nullable Vec3 sourcePosition() {
        if (this.boreEntityId == BoreDebrisParticleOptions.NO_ENTITY) {
            return new Vec3(
                    this.borePos.getX() + 0.5,
                    this.borePos.getY() + BlockEntityArcaneBore.EYE_HEIGHT,
                    this.borePos.getZ() + 0.5);
        }
