...
public final class BoreDrillFx {
    ...
    public static Vec3 tipOffset(float yawDegrees, float pitchDegrees, float eyeHeight) {
        ...
        return new Vec3(-Mth.sin(yaw) * horizontal, eyeHeight + TIP_VERTICAL_OFFSET * Mth.cos(pitch) - TIP_FORWARD_OFFSET * Mth.sin(pitch), Mth.cos(yaw) * horizontal);
    }

    public static void submit(PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera, Vec3 tip, float yawDegrees, float pitchDegrees, float beamUvScroll, float beamSpin, int tipFrame) {
        poseStack.pushPose();
        ...
        for (int strip = 0; strip < BEAM_STRIPS; strip++) {
            ...
            collector.submitCustomGeometry(poseStack, BoreBeamRenderType.DRILL_BEAM, (pose, buffer) -> addBeamVertices(buffer, pose.pose(), v0, v1));
        }
        poseStack.popPose();
        poseStack.pushPose();
        poseStack.translate(tip.x, tip.y, tip.z);
        poseStack.mulPose(camera.orientation);
        submitTipFlare(tipFrame, poseStack, collector);
        poseStack.popPose();
    }

    private static void addBeamVertices(VertexConsumer buffer, Matrix4fc matrix, float v0, float v1) {
        buffer.addVertex(matrix, 0.0F, 0.0F, -(float) BEAM_LENGTH).setUv(1.0F, v1).setColor(BEAM_TINT).setLight(BEAM_LIGHT);
        buffer.addVertex(matrix, -BEAM_RADIUS, 0.0F, 0.0F).setUv(1.0F, v0).setColor(BEAM_TINT).setLight(BEAM_LIGHT);
        buffer.addVertex(matrix, BEAM_RADIUS, 0.0F, 0.0F).setUv(0.0F, v0).setColor(BEAM_TINT).setLight(BEAM_LIGHT);
        buffer.addVertex(matrix, 0.0F, 0.0F, -(float) BEAM_LENGTH).setUv(0.0F, v1).setColor(BEAM_TINT).setLight(BEAM_LIGHT);
    }

    private static void submitTipFlare(int tipFrame, PoseStack poseStack, SubmitNodeCollector collector) {
