...
public final class EldritchLockRenderer implements BlockEntityRenderer<BlockEntityEldritchLock> {
    ...
    private void renderTablet(
            int count, Direction facing, PoseStack poseStack, MultiBufferSource buffers, int light, int overlay) {
        if (count < 0) {
            return;
        }
        ...
        poseStack.pushPose();
        poseStack.translate(
                0.5F + facing.getStepX() * TABLET_FACE_OFFSET,
                TABLET_HEIGHT,
