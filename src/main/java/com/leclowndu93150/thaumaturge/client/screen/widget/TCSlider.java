...
public final class TCSlider extends AbstractWidget {
    ...
    private void updateFromMouse(double mouseX, double mouseY) {
        float raw = vertical
                ? (float) (mouseY - (getY() + HANDLE_SIZE / 2.0)) / (height - HANDLE_SIZE)
                : (float) (mouseX - (getX() + HANDLE_SIZE / 2.0)) / (width - HANDLE_SIZE);
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (vertical) {
            graphics.pose().translate(getX() + TRACK_OFFSET, getY(), 0.0F);
            graphics.pose().scale(1.0F, height / (float) TRACK_LENGTH, 1.0F);
            graphics.blit(
            ...
            graphics.pose().translate(getX(), getY() + TRACK_OFFSET, 0.0F);
            graphics.pose().scale(width / (float) TRACK_LENGTH, 1.0F, 1.0F);
            graphics.blit(
                    TCScreenTextures.GUI_BASE,
                    0,
                    0,
