...
public final class ResearchTextEn {
    ...
    private static void part0(BiConsumer<String, String> add) {
        ...
        add.accept(
                "research.thaumaturge.ethereal_bloom.stage_0",
        ...
        add.accept("research.thaumaturge.primordial_nodes.title", "Primordial Node Manipulation");
        add.accept(
                "research.thaumaturge.primordial_nodes.stage_0",
                "I have developed a bit of a wild theory that might be worth exploring. Why not bring a primordial pearl into contact with an aura node?<BR>Sure, bringing two such sources of raw magical energy into contact with each other might be catastrophic, but their interaction could also be highly... interesting.<BR>While anyone can do this, I am sure that completing this research has slightly improved my chances of getting a good result. Very slightly.");
    ...
    private static void part1(BiConsumer<String, String> add) {
        ...
        add.accept("research.thaumaturge.essentia_crystalizer.title", "Essentia Crystallization");
        add.accept(
                "research.thaumaturge.essentia_crystalizer.stage_0",
                "I have nearly mastered the art of turning concrete objects into liquid essentia, so doing the reverse is a simple matter.<BR>The essentia crystallizer draws in liquid essentia from a pipe network and slowly converts it into small shards of crystalline essentia. Crystalline essentia is safe to handle and very stable.<BR>The Crystallizer either ejects the crystals into the world or into a container. It can be disabled with a redstone signal.<BR>Supplying it with vis will greatly increase its speed.");
        add.accept("research.thaumaturge.flux_scrubber.title", "Flux Scrubber");
        add.accept(
                "research.thaumaturge.flux_scrubber.stage_0",
                "Even the most skilled thaumaturge occasionally leaves a mess in the form of flux. It is a nuisance everyone has to deal with, but this machine removes the drudgery involved.<BR>The flux scrubber draws vis from the local aura and slowly removes flux goo within 16 blocks of where it is placed. What is more, it is able to reclaim a small portion of the magic that makes up the flux in the form of praecantatio essentia.<BR>Its internal buffer is very small and it will continue operating even when full, so it must be emptied if I wish to reclaim all the essentia.");
        ...
        add.accept(
                ...
                "Most types of essentia are fairly volatile and care must be taken in their storage - that is why most thaumaturges use warded jars. I, however, am not most thaumaturges.<BR>Using the wonderful new substance called void metal, I have discovered a way to greatly improve the storage of essentia. Essentia reservoirs are capable of storing up to 256 essentia in any combination.<BR>Potentially disastrous mixing of the different essentia types is prevented by dynamically reshaping compartments crafted from the new metal.");
    ...
    private static void part2(BiConsumer<String, String> add) {
        ...
        add.accept(
                ...
                "I have created designs for a mystical charm that pulses with healing energy that will wash all poisons or similar afflictions from my body. Unfortunately it cannot heal actual wounds.<BR>The charm has an internal store of vis that needs to be charged with a recharge pedestal or similar item - it cannot draw vis from the aura directly.<BR>There are also two specialized enchantments I can apply to the charm to improve its function.<LINE>§nLifegiver§r<BR>This infusion enchantment allows the Verdant Heart Charm to regenerate the health of whomever wears it.<LINE>§nSustainer§r<BR>This infusion enchantment allows the Verdant Heart Charm to slowly feed and sustain whomever wears it. This includes providing breathable air.");
    ...
    private static void part4(BiConsumer<String, String> add) {
        ...
        add.accept(
                "research.thaumaturge.warded_arcana.stage_0",
                "Some experimentation has led to an interesting discovery - the ability to imprint memories in inanimate objects. Your first attempt at using the discovery is doors, pressure plates and other blocks that remember who placed them.<BR>The door can only be removed with a wand by the person who placed it. Additionally the door will refuse to open for anyone but its owner. The owner can also craft iron or gold keys that will grant others access to this door.<BR>By crafting an iron key and clicking on the door, you will link the key to that door.<PAGE>You can then give the key to another person and they will be able to use it on the door to gain permanent permission to open it. They will still not be able to break the door, or grant anyone else access. For that you need the gold key.<BR>The gold key serves much the same function as the iron key, except they can also grant other people iron key access to that door. Only the original owner of the door can hand out gold key access. They are also not able to remove the door with a wand. Only the original owner will be able to do that.<PAGE>This pressure plate has a few novel features.<BR>Firstly it has 3 settings that can be toggled by right-clicking on it: It can trigger on everything (like a wooden pressure plate), just you, or everything except you.<BR>Secondly, like all other warded items only you can remove it.<BR>Lastly it can be linked to other players with the use of keys. These pressure plates can be used to open arcane doors as long as someone with access to the plate has access to the door as well.<PAGE>Warded glass is a simple cosmetic block, but like other warded objects it cannot be broken except by the person who placed it.<BR>Keys cannot be used to give other users access.");
        add.accept("research.thaumaturge.golem_fetter.title", "Golem Fetter");
        add.accept(
                "research.thaumaturge.golem_fetter.stage_0",
        ...
        add.accept(
                "research.thaumaturge.item_grate.stage_0",
