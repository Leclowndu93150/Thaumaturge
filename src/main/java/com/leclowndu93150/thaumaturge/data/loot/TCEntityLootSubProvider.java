...
public final class TCEntityLootSubProvider extends EntityLootSubProvider {
    ...
    public void generate() {
        ...
        add(
                TCEntities.PECH.get(),
                LootTable.lootTable().withPool(goldNuggetPool()).withPool(curioPool(TCItems.CURIO_KNOWLEDGE.get())));
        add(
                TCEntities.CULTIST_KNIGHT.get(),
                LootTable.lootTable().withPool(goldNuggetPool()).withPool(curioPool(TCItems.CURIO_RITES.get())));
        add(
    ...
    private LootPool.Builder brainPool() {
        return LootPool.lootPool()
                .setRolls(ConstantValue.exactly(1))
                .add(LootItem.lootTableItem(TCItems.BRAIN.get())
                        .when(LootItemRandomChanceWithEnchantedBonusCondition.randomChanceAndLootingBoost(
